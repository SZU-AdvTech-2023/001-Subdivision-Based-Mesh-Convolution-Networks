...
from mesh_tensor import MeshTensor, to_mesh_tensor

# 可能卷积消耗的临时空间过大，在这种情况下，可以关闭cudnn的临时内存申请
...
class MeshBatchNorm(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, is_train=False):
        super(MeshBatchNorm, self).__init__()
        self.bn = nn.BatchNorm2d(num_features, eps, momentum, is_train=is_train)
...
class MeshLeakyReLU(nn.Module):
    def __init__(self, negative_slope):
        super(MeshLeakyReLU, self).__init__()
        self.leaky_relu = nn.LeakyReLU(negative_slope)

    def execute(self, mesh_tensor: MeshTensor):
        feats = self.leaky_relu(mesh_tensor.feats)
        return mesh_tensor.updated(feats)


class MeshDropout(nn.Module):
    def __init__(self, p=0.5):
        super(MeshDropout, self).__init__()
...
class MeshLinear(nn.Module):
    def __init__(self, in_channels, out_channels, bias=True):
        super(MeshLinear, self).__init__()
        self.conv1d = nn.Conv1d(in_channels, out_channels, kernel_size=1, bias=bias)
...
if __name__ == '__main__':
    ...
    for meshes, labels, mesh_infos in tqdm(dataset):
        ...
        conv1 = MeshConv(13, 8, kernel_size=1, stride=2)
        meshes_tensor = conv1(meshes_tensor)
        print(meshes_tensor)
        break
